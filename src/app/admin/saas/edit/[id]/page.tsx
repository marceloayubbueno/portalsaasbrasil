...
export default function EditSaasPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          logo: data.logo || '',
          focusType: data.focusType || 'lead-generation',
          ...
          actionType: data.actionType || 'trial',
          actionUrl: data.actionUrl || '',
          buttonText: data.buttonText || 'Teste Grátis',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert('✅ Empresa SAAS atualizada com sucesso!')
        router.push('/admin/saas')
      } else {
        const error = await res.json()
        alert(`❌ Erro: ${error.message || 'Erro ao atualizar empresa'}`)
      }
    } catch (error) {
      console.error('Erro:', error)
      alert('❌ Erro ao atualizar empresa SAAS')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/saas">
              <button className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all text-sm font-medium">
                <X className="w-4 h-4" />
                ...
                <div className="h-full overflow-hidden">
                  ...
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border-2 border-green-200">
                    ...
                    <p className="text-sm text-green-700">
                      Revise as informações e clique em <strong>"Salvar Alterações"</strong> para finalizar a edição.
                    </p>
                  </div>
                    </div>
                  )}
                ...
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-3 border-2 border-amber-200">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-amber-800 leading-relaxed">
                      ...
                      {currentStep === 6 && 'Revise todos os dados antes de finalizar a edição.'}
                    </div>
                  </div>
                </div>
              </div>
