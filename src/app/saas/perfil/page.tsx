  ...
  const loadProfile = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Limpar valores temporários do pré-cadastro
        const cleanedData = {
          ...data,
          website: data.website === 'https://exemplo.com' ? '' : data.website,
          category: data.category === 'Outro' && data.description?.includes('Cadastro em andamento') ? '' : data.category,
          description: data.description?.includes('Cadastro em andamento') ? '' : data.description,
        }
  ...
  const handleSubmit = async () => {
    ...
    const token = localStorage.getItem('token')
      ...
      if (res.ok) {
        ...
        alert(`❌ Erro: ${error.message || 'Erro ao salvar perfil'}`)
      }
    } catch (error) {
      console.error('Erro:', error)
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Complete as informações da sua empresa</p>
                </div>
              </div>
            </div>
        ...
        <button
                  ...
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border-2 border-green-200">
                    ...
                    <p className="text-sm text-green-700">
                      Revise as informações e clique em <strong>"Salvar Perfil"</strong> para finalizar.
                    </p>
                  </div>
                    </div>
                  )}
